...
# Restaurer un logger détaillé avec fichier
...
logger.add("image_processor.log", level="DEBUG")
...
async def process_and_transfer_images() -> None:
    ...
    try:
        while True:
            try:
                ...
                try:
                    ...
                    if "network" in str(e).lower():  # Ne pas marquer comme traité en cas d’erreur réseau
                        continue
                    await source_db["realStateWithAgence"].update_one(
                        {"idSec": annonce_id},
                        {"$set": {"processed": True, "processed_at": datetime.utcnow()}}
        ...
        await close_db()  # Fermer la base de données à la fin
